...
**Partial evidence on the same question, from the joint model.** `ystar_ustar` has now been swept across this file's own `ratio_ystar` grid, and potential growth is the one thing that moves: 1.92 at 0, 1.94 at 0.05, 1.97 at 0.10, **1.99 at the default 0.13**, 2.14 at 0.25 and 2.27 at 0.40. So a threefold loosening of the level innovation lifts potential growth by roughly a third of a point, with the loose end heading toward Treasury's 2.5. Two qualifications. This is the joint model, not this one, though the potential block is shared code and both impose 0.078. And it is consistent with item 5 below rather than contradicting it: at the *tight* end the setting genuinely changes nothing, since 0 against 0.13 moves growth 0.07 here and 0.001 in this model. The action is entirely at the loose end of the grid, which item 5 never visited. `ystar`'s own `sigma_sweep.py` has still not been run on this spec, so the equivalent numbers for this model are unrecorded.

Worth noting what does *not* move in that sweep: the joint model's output gap is nearly invariant to it, because a wobblier potential competes with the GDP residual rather than with the gap. `sigma_e` falls 41% across the grid while `sigma_v` falls 9%. The inflation anchor is what protects the gap, and this is that mechanism seen from the other side.
...
Run with `./run-ystar.sh`; `--no-decompose` skips it, which is also the only way to run `--analyse-only` without touching ABS sources. Skipped automatically for the `labour` spec, which estimates the split internally.
...
Charts go to `charts/YStar-production/`, a separate directory because `run_analysis` clears its chart directory before writing and the two specifications would otherwise delete each other's output.
...
Charts are written to `charts/YStar/`: `potential-growth`, `output-gap`, `output-gap-composition`, `potential-growth-hours-and-productivity`, `potential-growth-and-labour-input`, `contributions-to-potential-growth`, and `gdp-and-potential-output` and `actual-growth-versus-potential` on full and recent windows.
...
./run-ystar.sh --anchor 2.25    # move the anchor
./run-ystar.sh --ratio-ystar 0.25   # looser potential
...
./run-ystar.sh --ar1-residual --prefix ystar_ar1        # AR(1) residual (item 15)
./run-ystar.sh --two-sided-c --prefix ystar_pub2sided   # c's sign estimated (item 17)
./run-ystar.sh --zero-deviation 2020Q2 2021Q1 --prefix ystar_zerodev  # lockdown carries no d (item 16)
./run-ystar.sh --spec production --no-mfp-observation --sigma-gm 0.015 \
    --prefix ystar_nomfp   # drop the MFP equation that double-counts GDP
./run-ystar.sh --spec production --ratio-a 0.05 --prefix ystar_alpha  # let alpha drift
./run-ystar.sh --analyse-only --no-decompose  # charts from the trace alone, no ABS access
```
...
Traces retained as `ystar_sweep_ratio_g_*`.
...
**Not adopted, and the reason is structural rather than statistical.** Serial correlation in `e_c` is what an omitted IS curve and expectations block look like. AR(1) is a one-parameter reduced form for that structure, and a poor one, since policy transmission runs six to eight quarters rather than decaying geometrically from one. Adopting it would smuggle back in exactly what this package was built to do without, and would do so badly. The option is kept off by default, in the same spirit as `free_sigma_ystar`: to make the check reproducible, not because the alternative is a candidate. Trace retained as `ystar_ar1`.
