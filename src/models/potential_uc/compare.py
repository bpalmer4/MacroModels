"""Compare saved potential_uc runs on one set of axes.
...
- `potential_uc`          quarterly (non-overlapping) inflation, no supply term
- `potential_uc_supply`   the same, plus demeaned import price growth
- `potential_uc_piannual` the original four-quarter (overlapping) inflation
...
Usage::
    ...
    uv run python -m src.models.potential_uc.compare
    uv run python -m src.models.potential_uc.compare --runs potential_uc potential_uc_supply
"""
...
from src.models.potential_uc.results import DEFAULT_CHART_BASE, load_results

# Own directory, for the same reason as `realtime.CHART_DIR`: `run_analysis`
# clears PotentialUC/ before writing.
CHART_DIR = DEFAULT_CHART_BASE / "PotentialUC" / "Compare"

DEFAULT_RUNS: dict[str, str] = {
    "potential_uc": "Quarterly inflation",
    "potential_uc_supply": "+ import prices",
    "potential_uc_piannual": "Annual inflation (overlapping)",
}
...
def main() -> None:
    """CLI entry point."""
    parser = argparse.ArgumentParser(description="Compare saved potential_uc runs")
    parser.add_argument(
