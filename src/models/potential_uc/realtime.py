...
Usage::
    ...
    uv run python -m src.models.potential_uc.realtime --first 2005Q4 --step 4
"""
...
from src.models.potential_uc.base import SamplerConfig, sample_model
from src.models.potential_uc.config import ModelConfig
from src.models.potential_uc.estimate import build_model
from src.models.potential_uc.observations import build_observations
from src.models.potential_uc.results import DEFAULT_CHART_BASE, PotentialResults

# Its own directory: `analyse.run_analysis` clears PotentialUC/ before writing,
# so charts left there by a separate command would not survive the next run.
CHART_DIR = DEFAULT_CHART_BASE / "PotentialUC" / "RealTime"
...
def main() -> None:
    """CLI entry point."""
    parser = argparse.ArgumentParser(description="Pseudo-real-time revisions for potential_uc")
    ...
    parser.add_argument("--prefix", default="potential_uc")
    parser.add_argument(
