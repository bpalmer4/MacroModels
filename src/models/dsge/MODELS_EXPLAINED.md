...
> **Status: experimental / in development, NOT production-ready.** These are research and diagnostic builds. As of the Bayesian re-estimation (`fa_nk_bayes.py`) the **Taylor block is now data-identified** (φ_π≈2.6, an aggressive inflation-response rule) and both FA-NK and FA-NK-wage **converge cleanly** (r̂≤1.01). But the **r\*** and **NAIRU/U\*** these models produce are still **not credible**: for those use the purpose-built `rstar_hlw` and `nairu` models. The φ_π result's robustness (e.g. with the financial block removed) has **not yet been tested**. Treat outputs as research artefacts, not operational estimates.
...
Two extensions: a **Tier-1 labour block** (structural marginal cost, behind `FANKModel(labour_block=True)`) and **`FA-NK-wage`** (`fa_nk_wage_model.py`), which adds sticky wages and Galí-2011 unemployment. Key honest findings across the family: the financial wedge is essentially an exogenous shock that tracks the **global financial cycle** (not QE narrowly - see `omega_global_test.py`); the labour block **unsticks κ_p**; the **Taylor block is identified under Bayesian estimation** (`fa_nk_bayes.py`: the MLE φ_π-pins-at-cap was a maximiser artefact - the posterior locates φ_π≈2.6, an aggressive rule); and a **credible NAIRU is out of reach** here (no NAIRU random-walk state - use `src/models/nairu/`).

## Summary Table

| Model | Type | Phillips Curve | Outputs | Status |
|-------|------|----------------|---------|--------|
| **FA-NK** | True DSGE + financial accelerator | κ_p × y_gap (or structural mc if labour_block) | Two r* (safe, capital), endogenous EFP wedge | Determinate; χ small (wedge ~exogenous). Taylor block **identified under Bayesian** (`fa_nk_bayes.py`, φ_π≈2.58, r̂=1.03); weakly identified only under MLE |
| **FA-NK-wage** | FA-NK + sticky wages + Galí unemployment | κ_p × mc, κ_w × μ_w | + unemployment gap, real wage, U* | Determinate; Bayesian run fully converged (r̂≤1.01, φ_π≈2.40, κ_w≈0.40); but U*/NAIRU not credible (volatile) |
...
- **The Taylor block looks weakly identified *under MLE* (later overturned, see the Bayesian re-estimation below).** With the original bounds φ_π, φ_y, κ_p peg at their ceilings; widening the bounds shows them teleporting to different corners (φ_y to 0 or to whatever ceiling) at near-identical likelihood. This *looked* like flat-ridge non-identification, but the Bayesian posterior shows it was a maximiser artefact on a likelihood that is informative-but-flat-topped above φ_π≈2, the policy block *is* identified once you map the posterior rather than chase the peak. The MLE production run uses economically-reasonable caps (φ_π≤3, φ_y≤1) as effectively-imposed priors. χ (≈0.012-0.024) and the financial block are stable by contrast. (φ_π *does* come off its cap once unemployment is observed - see FA-NK-wage.)
...
**Bayesian re-estimation (DONE - resolves the weak-identification thread).** `fa_nk_bayes.py` wraps the existing numpy likelihood (Blanchard-Kahn solve + Kalman filter) as a black-box pytensor `Op`, puts explicit priors on the estimated parameters (φ_π ~ TruncatedNormal(1.5, 0.25, lower=1); financial/persistence/volatility blocks weakly informative and bounded by construction so they *replace* the MLE caps), and samples gradient-free with DEMetropolis-Z. **Run:** `uv run python -m src.models.dsge.fa_nk_bayes` (add `--smoke` for a quick wiring check); writes `model_outputs/fa_nk_bayes_params_{fa_nk,wage}.txt`, the InferenceData `.nc`, and prior-vs-posterior / forest charts to `charts/dsge-fa-nk-bayes/`.

*Result:* the Taylor block is **identified, not flat** - the MLE ceiling-pegging was masking a genuine, data-driven preference for an *aggressive* inflation response. Core FA-NK converged cleanly (max r̂=1.03, min ESS≈283) with **φ_π posterior ≈ 2.58 (94% HDI [2.27, 2.92])**, a near-complete shift off the 1.5 prior with almost no prior/posterior overlap (contraction 0.45); φ_y pulls down to ≈0.14. The financial block survives the sanity check (χ posterior ≈0.03, HDI includes the MLE ~0.017). FA-NK-wage tells the same Taylor story (φ_π≈2.40, κ_w≈0.40) and, after a longer run (tune=15000, draws=15000, 4 chains), is **also fully converged** (max r̂=1.01, min ESS≈813; the earlier ρ_supply/ρ_wage under-mixing at ESS≈12 was just chain length, not an un-traversable ridge - the κ_p–σ_supply correlation is ≈0.91 and ρ_wage–σ_wage ≈−0.67, but DEMetropolis-Z crosses them given enough history). The earlier "φ_π pins at its cap" reading was a maximiser artefact on a likelihood that is informative but flat-topped above ~2; the posterior locates the mass.

**Posterior state extraction (DONE).** `produce_fa_nk_extractions()` (also `--extract-only`) draws 400 parameter sets from the FA-NK posterior, runs the Kalman smoother for each, and bands the smoothed latent series by the 16/84th percentile - the natural rate r*, the safe-rate-vs-cost-of-capital divergence, and the EFP wedge, written to `charts/dsge-fa-nk-bayes/` and `model_outputs/fa_nk_bayes_states.csv`. This is the band the MLE byproduct could not produce: it folds in **parameter** uncertainty. (The r* shape turns out robust to it - the band is tight. NB: this is parameter uncertainty only; full predictive bands would also add the within-draw Kalman state covariance.)

**Charts** are all mgplot: the KDE prior-vs-posterior overlays and the HDI forest are drawn as content on an Axes then finalised via `finalise_plot`; the banded extraction charts layer `fill_between_plot` + `line_plot` + `finalise_plot`.

**Open threads:** Optional: full predictive r*/EFP bands (convolve parameter + within-draw state uncertainty), if the tight parameter-only band understates true uncertainty.
