...
def decompose_inflation(
    ...
    excess, has_excess = get_excess_contribution(trace, obs, obs_index, "beta_pi")
    demand = gamma_pi * (U - nairu) / U
    supply_import = rho_pi * delta_4_pm_lag1
    supply_gscpi = xi_gscpi * gscpi ** 2 * np.sign(gscpi)
    fitted = anchor + excess + demand + supply_import + supply_gscpi
    residual = pi_observed - fitted

    return InflationDecomposition(
        observed=pi_observed, anchor=anchor, excess=excess, demand=demand,
