...
- **Known wrinkle**: at 1993Q1–Q2 the series reads −0.29/−0.56 then flips sign,   residual estimator disagreement leaking through the switch-on. Two observations
...
- **`simple_excess_nohcoe`** (drop the collinear hourly-COE wage curve). The
  motivating hypothesis, that HCOE collinearity dilutes the weak ULC wage β,   is **refuted**: `beta_wg` is unchanged (0.460 → 0.463, CI still straddles 0),
...
- **`simple_excess_tprice`, `simple_excess_regime_tprice`** (Student-t price
  ...
  **Improves reliability but not fit.** Price-eq Pareto-k cleans up fully (max k
  ...
  (Δ≈0.03). Reason: estimated `nu_pi` ≈ 21 (90% CI [8, 52], P(ν<10)≈0.10),   barely off its prior mean of 20, so the data don't demand fat tails (one
...
ranking and default stand. The 2022Q2 high-k point is genuinely just hard data, no richer NAIRU-innovation or error model exploits it.
